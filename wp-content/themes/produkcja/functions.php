...
function get_head( $name = null, $args = array() ) {
	/**
...
function get_head_post( $name = null, $args = array() ) {
	/**
	 * Fires before the header template file is loaded.
	 *
	 * @since 2.1.0
	...
	$name      = (string) $name;
	if ( '' !== $name ) {
		$templates[] = "head-post-{$name}.php";
	}

	$templates[] = 'head-post.php';
...
function wp_nav_menu1( $args = array() ) {
...
function new_submenu_class($menu) {    
    $menu = preg_replace('/ class="sub-menu"/','/ class="navbar-dropdown" /',$menu);        
    return $menu;      
}
